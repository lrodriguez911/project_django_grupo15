...
from .views import RegisterView, CustomLoginView  , ResetPasswordView , ChangePasswordView , profile
...
urlpatterns = [
    ...
    path('register/', RegisterView.as_view(template_name='pacientes/register.html'), name='users-register'),
    
    path('login/', CustomLoginView.as_view(redirect_authenticated_user=True, template_name='pacientes/login.html',
                                           authentication_form=LoginForm), name='login'),
    path('logout/', auth_views.LogoutView.as_view(template_name='pacientes/logout.html'), name='logout'),
    path('password-reset/', ResetPasswordView.as_view(template_name='pacientes/password_reset.html'), name='password_reset'),
    path('password-reset-confirm/<uidb64>/<token>/',
         auth_views.PasswordResetConfirmView.as_view(template_name='pacientes/password_reset_confirm.html'),
         name='password_reset_confirm'),
    path('password-reset-complete/',
         auth_views.PasswordResetCompleteView.as_view(template_name='pacientes/password_reset_complete.html'),
